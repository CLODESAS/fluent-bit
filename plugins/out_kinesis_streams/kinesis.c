                              ...
                              struct flb_config *config, void *data)
{
    ...
    tmp = flb_output_get_property("stream_arn", ins);
    if (tmp) {
        ctx->stream_arn = tmp;
        /*
         * If only stream_arn is provided, derive stream_name from the ARN
         * so existing logging and aggregation paths keep working. ARN format:
         * arn:aws:kinesis:<region>:<account>:stream/<name>
         */
        if (!ctx->stream_name) {
            const char *slash = strrchr(tmp, '/');
            if (slash && *(slash + 1) != '\0') {
                ctx->stream_name = slash + 1;
            }
        }
    }

    if (!ctx->stream_name && !ctx->stream_arn) {
        flb_plg_error(ctx->ins,
                      "either 'stream' or 'stream_arn' must be configured");
        goto error;
    }
...
static struct flb_config_map config_map[] = {
    ...
    {
     FLB_CONFIG_MAP_STR, "stream_arn", NULL,
     0, FLB_TRUE, offsetof(struct flb_kinesis, stream_arn),
     "Full ARN of the Kinesis Data Stream. When set, the PutRecords request "
     "uses 'StreamARN' instead of 'StreamName'. Required for cross-account "
     "writes that rely on a resource-based policy on the destination stream."
    },
