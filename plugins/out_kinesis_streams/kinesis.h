...
struct flb_kinesis {
    ...
    /*
     * Optional Stream ARN. When set, the PutRecords payload uses the
     * "StreamARN" field instead of "StreamName". Required for cross-account
     * Kinesis writes governed by a resource-based policy on the destination
     * stream (the AWS C SDK otherwise builds the ARN with the caller's
     * account, which bypasses the resource policy).
     */
    const char *stream_arn;
    const char *time_key;
