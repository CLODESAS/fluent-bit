 ...
 *
 * If stream_arn is configured, emits {"StreamARN":"..."}, otherwise emits
 * {"StreamName":"..."}. StreamARN is required for cross-account writes
 * authorized by a resource-based policy on the destination stream.
 */
...
static void reset_flush_buf(struct flb_kinesis *ctx, struct flush *buf) {
    ...
    if (ctx->stream_arn) {
        buf->data_size += strlen(ctx->stream_arn);
    }
    else {
